...
const PRIVACY_DATA = {
    ...
    sections: [
      {
        title: '纯本地驱动与零数据收集',
        paragraphs: [
          '糖葫芦墨记是一款专为 macOS 设计的本地离线 Markdown 编辑器。我们郑重声明：本应用没有任何云端服务器，绝对不会收集、存储、处理或上传您的任何个人信息、文档正文内容、笔记数据或应用使用记录。',
          '本应用完全不包含任何第三方广告 SDK、用户画像追踪器（Trackers）或行为分析工具（Analytics）。'
        ]
      },
      {
        title: '数据与文件系统访问权限',
        paragraphs: [
          '本应用遵循“最少权限”与“主动触发”原则：'
        ],
        list: [
          { label: '文件读取与保存', detail: '仅在您主动使用“打开文件”、“保存文档”、“导出为 PDF/HTML”或“粘贴图片”等功能时，才会访问您明确指定的本地文件或目录。' },
          { label: '零后台扫描', detail: '应用绝不会在后台索引、扫描或读取您硬盘上未经授权的任何文件或文件夹。' },
          { label: '数据完全归您所有', detail: '您创建的所有 Markdown 文档、缓存文件及关联图片均仅保存在您本地指定的磁盘目录中。' }
        ]
      },
      {
        title: '网络请求场景说明',
        paragraphs: [
          '糖葫芦墨记在绝大多数场景下完全无需网络即可离线运行。仅在以下特定功能开启时可能产生必要的网络请求：'
        ],
        list: [
          { label: '外部图表渲染依赖', detail: '当文档包含复杂 Mermaid 图表时，内置预览引擎可能从公共安全 CDN 节点轻量加载基础渲染脚本，此过程不发送任何用户隐私数据。' },
          { label: '可选第三方图床服务', detail: '如果您主动在设置中开启并配置了 PicGo 等第三方图床，应用将根据您的个人配置将图片上传至您指定的图床服务器，全过程由您完全掌控。' }
        ]
      },
      {
        title: '无需注册与数据清除保障',
        paragraphs: [
          '本应用提供开箱即用的完整体验，无需注册任何账户，无需绑定邮箱或手机号。',
          '当您删除本地文档或卸载本应用时，存储于设备本地私有目录中的一切相关数据将被操作系统彻底清除，无任何残留。'
        ]
      }
    ]
  },
    ...
    sections: [
      {
        title: '个人信息与内容隐私',
        paragraphs: [
          '糖葫芦视界作为第三方流媒体客户端，我们绝不收集、上传或与任何人共享您的观看历史、搜索记录及个人偏好等隐私数据。',
          '所有的算法推荐数据获取与本地缓存记录均保存在您的 Apple TV 设备沙盒内，我们无法访问。'
        ]
      },
      {
        title: '扫码登录数据说明',
        paragraphs: [
          '当您使用手机扫码登录时，应用仅与源平台官方服务器直接交互获取授权令牌（Token），我们不设立任何中间服务器进行数据拦截或存储。您的账号安全完全受到官方认证体系的保护。'
        ]
      },
      {
        title: '系统权限申请与用途',
        paragraphs: [
          '为了保障核心视频流的加载与播放体验，我们需要您授权以下基础权限：'
        ],
        list: [
          { label: '网络访问权限', detail: '用于连接互联网、加载视频流以及进行相关授权认证。' }
        ]
      }
    ]
  },
  'thl-shot': {
    ...
    contactEmail: 'support@thltv.com',
    badges: ['适用于 macOS 操作系统', '纯端侧运行与离线推理', '无广告·无跟踪·零云端上传'],
    intro: '糖葫芦截屏（Thlshot）是一款本地运行的 macOS 屏幕截图与标注工具。我们尊重您屏幕上的所有内容，绝对不会为广告、分析或训练模型上传截图、OCR 文本或翻译文本。请您在使用本软件前仔细阅读本政策。',
    sections: [
      {
        title: '屏幕录制权限申请与使用',
        paragraphs: [
          '只有在您通过自定义全局快捷键、状态栏菜单或应用内界面操作主动发起截图时，应用才会读取当前屏幕画面。',
          'macOS 系统会对屏幕录制权限进行严格安全管控，您可在任意时间于“系统设置 → 隐私与安全性 → 屏幕录制”中主动查看或撤销授权。'
        ],
        list: [
          { label: '触发机制', detail: '严格仅限用户主动发起截图时读取画面，无任何静默或后台常驻录屏行为。' },
          { label: '权限撤销', detail: '您可以随时在 macOS 系统设置中关闭屏幕录制权限，关闭后应用将无法捕获画面。' }
        ]
      },
      {
        title: '截图图像、离线 OCR 与即时翻译',
        paragraphs: [
          '截取的图像仅在您的 Mac 本机内存中处理与渲染。',
          '文字识别 (OCR) 依托 Apple Vision 原生深度学习框架直接在设备端芯片运行；在受支持的系统版本上，多语言翻译依托 Apple Translation 与系统已下载的离线语言包完成。',
          '本应用绝不将您的截屏图像、提取文字或翻译结果发送到任何第三方或我们自有的云端服务器。'
        ],
        list: [
          { label: '端侧推理', detail: '基于 Apple Vision 框架完全离线运行，无需连网，无需配置外部 API 密钥。' },
          { label: '数据不上传', detail: '您的屏幕画面、个人文档及敏感情报 100% 留存在您的 Mac 本地。' }
        ]
      },
      {
        title: '本地文件存储与系统剪贴板',
        paragraphs: [
          '只有当您主动点击“保存”按钮时，应用才会将 PNG 或 JPEG 图像写入您选定的本地文件系统目录中。',
          '当您主动选择“复制”或双击选区时，图片数据将写入 macOS 系统剪贴板；取消截图操作不会修改或覆盖您现有的剪贴板内容。截图会话过程中产生的临时图形缓存均会在会话结束后立即自动彻底清理。'
        ]
      },
      {
        title: '本地偏好设置管理',
        paragraphs: [
          '您所配置的全局快捷键、图像默认保存目录、鼠标指针显示、开机自启动及贴图悬浮窗状态等偏好项，仅以轻量配置文件保存在您的 Mac 本地沙盒目录中。',
          '当前版本不启用任何匿名分析、广告追踪（Trackers）或强制用户账户系统，开箱即用。'
        ]
      },
      {
        title: '系统级分享服务说明',
        paragraphs: [
          '当您在标注栏中主动点击“分享”功能时，应用将调起 macOS 系统原生分享面板。',
          '您在系统面板中所选择的外部分享渠道（如邮件、隔空投送或第三方即时通讯软件）由 Apple 或相应第三方服务商提供，其后续数据处理均适用对应服务的隐私政策。'
        ]
      },
      {
        title: '崩溃日志与使用数据',
        paragraphs: [
          '当前版本未接入任何第三方崩溃上报（Crash Report）或行为数据埋点分析 SDK。',
          '如未来版本为了提升软件稳定性而计划引入相关能力，我们将在正式启用前更新本隐私政策，并提供明确的开关供您自主选择是否开启。'
        ]
      },
      {
        title: '政策更新与联系方式',
        paragraphs: [
          '若本应用的功能逻辑或数据处理方式发生实质调整，我们将在官网及应用发布说明中及时公布更新后的隐私政策。',
          '如果您对本隐私政策或个人信息保护有任何疑问、意见或反馈，欢迎随时与我们取得联系：support@thltv.com。'
        ]
      }
    ]
  }
